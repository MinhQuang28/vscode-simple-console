...
function findEnclosingNameRegex(
  doc: vscode.TextDocument,
  line: number,
): string {
  const fnRe =
    /(?:function\s+([A-Za-z_$][\w$]*)|(?:const|let|var)\s+([A-Za-z_$][\w$]*)\s*=\s*(?:async\s*)?(?:\([^)]*\)|[A-Za-z_$][\w$]*)\s*=>|([A-Za-z_$][\w$]*)\s*\([^)]*\)\s*\{|class\s+([A-Za-z_$][\w$]*))/;
...
async function insertLog() {
  ...
  for (const sel of editor.selections) {
    ...
    const nextLine =
      endLine + 1 < doc.lineCount ? doc.lineAt(endLine + 1).text : "";
    const dupRe = new RegExp(
      `console\\.log\\(["'\`]${escapeRe(emoji)} ~ [^~]+ ~ ${escapeRe(varName)}:`,
    );
  ...
  if (!jobs.length) {
    vscode.window.showInformationMessage(
      "Quick Log: nothing to insert (no var / duplicate).",
    );
    return;
  }
  ...
  await editor.edit((eb) => {
    for (const j of jobs) {
      const pos = new vscode.Position(
        j.endLine,
        doc.lineAt(j.endLine).text.length,
      );
...
async function removeAllLogs() {
  ...
  for (let i = 0; i < doc.lineCount; i++) {
    const t = doc.lineAt(i).text;
    if (LOG_RE.test(t) || COMMENTED_LOG_RE.test(t))
      ranges.push(doc.lineAt(i).rangeIncludingLineBreak);
  }
  if (!ranges.length) {
    vscode.window.showInformationMessage("Quick Log: no logs found.");
    return;
  }
  await editor.edit((e) => ranges.reverse().forEach((r) => e.delete(r)));
  vscode.window.showInformationMessage(
    `Quick Log: removed ${ranges.length} log(s).`,
  );
  updateStatus();
}
...
async function commentAllLogs() {
  ...
  for (let i = 0; i < doc.lineCount; i++) {
    ...
    if (m)
      edits.push({ line: i, text: `${m[1]}// ${m[2]}`, range: line.range });
  }
  if (!edits.length) {
    vscode.window.showInformationMessage(
      "Quick Log: no active logs to comment.",
    );
    return;
  }
  await editor.edit((e) => edits.forEach((x) => e.replace(x.range, x.text)));
  vscode.window.showInformationMessage(
    `Quick Log: commented ${edits.length} log(s).`,
  );
  updateStatus();
}
...
async function uncommentAllLogs() {
  ...
  if (!edits.length) {
    vscode.window.showInformationMessage("Quick Log: no commented logs found.");
    return;
  }
  await editor.edit((e) => edits.forEach((x) => e.replace(x.range, x.text)));
  vscode.window.showInformationMessage(
    `Quick Log: uncommented ${edits.length} log(s).`,
  );
  updateStatus();
}
